...
def test_scan_and_build_trial_index_with_separate_pose_root(tmp_path: Path) -> None:
    ...
    assert trial_rows[0]["object_names"] == "peg"
    assert trial_rows[0]["trial_success_all_actions"] is True
    assert trial_rows[0]["trial_success_last_action"] is True
...
def test_build_trial_index_infers_object_name_and_success_from_segments(tmp_path: Path) -> None:
    raw_root = tmp_path / "raw" / "data"
    raw_root.mkdir(parents=True)
    pose_root = tmp_path / "raw" / "poses"
    pose_root.mkdir(parents=True)
    ...
    with h5py.File(h5_path, "w") as handle:
        ...
        segments = handle.create_group("segments_info")
        no_action = segments.create_group("0")
        no_action.create_dataset("start", data=0.0)
        no_action.create_dataset("end", data=0.1)
        no_action.create_dataset("text", data="No action.")
        no_action.create_dataset("success", data=True)

        pick = segments.create_group("1")
    ...
    assert trial_rows[0]["object_names"] == "Ethernet"
    assert trial_rows[0]["trial_success_all_actions"] is True
    assert trial_rows[0]["trial_success_last_action"] is True
...
def test_build_trial_index_marks_trial_failed_when_any_action_fails(tmp_path: Path) -> None:
    ...
    assert trial_rows[0]["object_names"] == "Ethernet"
    assert trial_rows[0]["trial_success_all_actions"] is False
    assert trial_rows[0]["trial_success_last_action"] is False


def test_build_trial_index_distinguishes_all_actions_from_last_action_success(tmp_path: Path) -> None:
    raw_root = tmp_path / "raw" / "data"
    raw_root.mkdir(parents=True)
    pose_root = tmp_path / "raw" / "poses"
    pose_root.mkdir(parents=True)
    ...
    with h5py.File(h5_path, "w") as handle:
        ...
        pick.create_dataset("end", data=0.5)
        pick.create_dataset("text", data="Pick Ethernet.")
        pick.create_dataset("success", data=False)
    ...
    pose_path = pose_root / "trial_004_poses.json"
    pose_path.write_text(json.dumps({"Hama1": [[0, 0, 0], [0, 0, 0, 1]]}), encoding="utf-8")
